...
//  SpaceShooter
//
//  Created by Никита Долгих on 01/03/2023.
//
...
//        player.setScale(2)
        ...
        scoreLabel.position = CGPoint(x: 100, y: UIScreen.main.bounds.height - 75)
        score = 0
        ...
        motionManager.startAccelerometerUpdates(to: OperationQueue.current!) {(data: CMAccelerometerData?, error: Error?) in
            if let accelerometrData = data{
                ...
                self.xAccelarate = CGFloat(acceleration.x) * 0.75 + self.xAccelarate * 0.25
            }
        }
    }
    
    override func didSimulatePhysics() {
        player.position.x += xAccelarate * 50
...
//        alien.setScale(2)
        ...
        var animDuration:TimeInterval = 6
